        ...
        public List<ComicStrip> GetAll()
        {
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    return table.AsEnumerable().Select(s => new ComicStrip(s.Field<int>("Id"), s.Field<string>("Title"), (s.Field<int>("Serie_Id") > 0) ? GetSerie(s.Field<int>("Serie_Id")) : null, s.Field<int>("Number"), this.GetAuthors(s.Field<int>("Id")), publishers.Where(x => x.ID == s.Field<int>("Publisher_Id")).SingleOrDefault())).ToList<ComicStrip>();
                }
            }
            catch (Exception) { throw new QueryException(); }
